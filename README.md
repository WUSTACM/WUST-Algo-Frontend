...
- 后台管理：用户管理、角色/分组调整、密码重置、抓取任务查看、统计缓存管理、教练/管理员群发站内消息、系统邀请码配置。
...
- `提交记录`：提交 ID、状态、时间、语言、是否计入 AC。
- `抓取审计`：本次抓取条数、AC 去重数、提交去重数、异常/跳过记录数和最近错误。

后台系统页提供统计缓存可视化和清理入口。管理员/教练可以查看全站或指定用户缓存状态，并清理统计缓存来排查“数字一会变”的问题。
