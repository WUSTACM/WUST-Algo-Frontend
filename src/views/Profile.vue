<template>
  <BaseLayout>
        ...
        <div class="left" style="position: relative">
          ...
          <div class="info">
            ...
            <div class="details" v-if="user">
              <div class="item" v-for="oj in ojPlatforms" :key="oj.key">
                ...
                <div
                  class="link"
                  :class="
                    !(!user.links[oj.key] && currentUserId !== Number(user.userId)) ? 'go' : ''
                  "
                >
                  <div v-if="!user.links[oj.key] && currentUserId !== Number(user.userId)">
                    未绑定
                  </div>
                  <router-link
                    v-else-if="!user.links[oj.key] && currentUserId === Number(user.userId)"
                    :to="`/changeProfile?oj=${oj.key}`"
                    >去绑定</router-link
                  >
            ...
            <div class="moblie-details">
              <a class="item" v-if="user.links.NowCoder" target="_blank" :href="user.links.NowCoder"
                >牛客</a
              >
              <a class="item" v-if="user.links.AtCoder" target="_blank" :href="user.links.AtCoder"
                >AtCoder</a
              >
              <a
                class="item"
                v-if="user.links.CodeForces"
                target="_blank"
                :href="user.links.CodeForces"
                >CodeForces</a
              >
              <a class="item" v-if="user.links.LuoGu" target="_blank" :href="user.links.LuoGu"
                >洛谷</a
              >
          ...
          <div class="actions">
            ...
            <button
              v-else-if="canShowProfileMessageAction"
              class="btn def"
          ...
          <div class="team-card" style="position: relative">
            ...
            <div class="team-header">
              <div class="team-title">
                <img
                  class="team-avatar"
                  :src="teamInfo.avatar || '/images/defaultAvatar.png'"
                  alt=""
                />
                <div>
              ...
              <button
                v-if="isSelfProfile && teamInfo.id === 0 && teamPanelMode === 'idle'"
                class="team-edit"
              ...
              <button
                v-else-if="canManageTeam && teamPanelMode === 'idle'"
                class="team-edit"
                @click="startEditTeam"
              >
                编辑团队
              ...
              <button
                v-else-if="canLeaveTeam && teamPanelMode === 'idle'"
                class="team-edit danger"
                @click="leaveTeam"
              >
                退出团队
              </button>
              <button
                v-else-if="teamPanelMode !== 'idle'"
                class="team-edit"
            ...
            <div v-else-if="teamPanelMode === 'edit'" class="team-form">
              ...
              <div class="team-edit-members">
                ...
                >
                  ...
                  >
                    ...
                    <button class="team-action danger" @click="removeMember(member.userId)">
                      移除
                    </button>
                  </div>
                </div>
              ...
              <div class="team-candidates" v-if="inviteCandidates.length > 0">
                <button
                  class="team-candidate"
                  v-for="candidate in inviteCandidates"
                  :key="candidate.userId"
                  @click="sendInvite(Number(candidate.userId))"
                >
            ...
            <div
              v-if="teamOwner"
              class="team-owner"
              @click="router.push(`/profile?id=${teamOwner.userId}`)"
            >
            ...
            <div v-if="teamInfo.id !== 0" class="team-dashboard">
              ...
              <div class="team-dashboard-members" v-if="teamDashboard.members.length > 0">
                <div
                  class="team-dashboard-member"
                  v-for="member in teamDashboard.members.slice(0, 5)"
                  :key="member.userId"
                >
            ...
            <div class="team-members">
              <div
                class="team-member"
                v-for="member in teamInfo.members.slice(0, 6)"
                :key="member.userId"
              ...
              <div v-if="!loadingTeam && teamInfo.members.length === 0" class="team-empty">
                暂无团队成员
              </div>
            </div>
          </div>
        ...
        <div class="right">
          <div class="stat-scope-card">
            <div class="stat-scope-note">
              ...
              <span>{{
                statisticExplanation.summary ||
                "本站统计基于提交日志统一去重，可能与 OJ 主页展示口径不同。"
              }}</span>
              <button
                class="inline-link-button"
                @click="showStatisticExplanation = !showStatisticExplanation"
              >
          ...
          <div class="section" style="position: relative">
            ...
            <div class="header">
              ...
              <div class="header-tabs">
                <span
                  class="tab"
                  @click="currentData = 0"
                  :class="currentData === 0 ? 'active' : ''"
                  >提交</span
                >
                <span
                  class="tab"
                  @click="currentData = 1"
                  :class="currentData === 1 ? 'active' : ''"
                  >AC</span
                >
              </div>
            </div>
            <div class="content">
              <div v-if="currentData === 0" class="statisticItems">
                <div class="statisticItem" v-for="item in data.submit" :key="item.title">
                  ...
                  <div class="barContainer">
                    ...
                    <div
                      class="ave"
                      :style="'left:' + (item.ave / (item.value + item.ave)) * 100 + '%'"
                    >
                      平均值:{{ item.ave.toFixed(2) }}
                    </div>
                    <div
                      class="cursor"
                      :style="'left:' + (item.ave / (item.value + item.ave)) * 100 + '%'"
                  ...
                  <div class="value">
                    {{ item.value.toString().includes(".") ? item.value.toFixed(2) : item.value }}
                  </div>
                  <div class="grow" :class="getGrowClass(item.grow)">
                    {{ item.grow ? (item.grow > 0 ? "+" + item.grow : item.grow) : "" }}
                  </div>
                </div>
              </div>
              <div v-else class="statisticItems">
                <div class="statisticItem" v-for="item in data.ac" :key="item.title">
                  <div class="title">{{ item.title }}</div>
                  <div class="barContainer">
                    <div
                      class="bar"
          ...
          <div class="section insight-section" style="position: relative">
            ...
            <div class="content">
              ...
              <div
                v-if="activeSpiderJob"
                class="spider-job-card"
                :class="`job-${activeSpiderJob.status}`"
              >
                <div>
                ...
                <div class="job-progress">
                  <div
                    ...
                    :style="{ width: jobProgress(activeSpiderJob) + '%' }"
                  ></div>
                </div>
              </div>
              <div class="sync-status-list">
                <template v-if="syncStatusRows.length > 0">
                  <div class="sync-status-item" v-for="item in syncStatusRows" :key="item.platform">
                    <div>
                      ...
                      <div class="sync-meta">
                        @{{ item.username }} · {{ formatSyncTime(item.lastSuccessAt) }}
                      </div>
                      <div v-if="isCodeforcesPlatform(item.platform)" class="cf-refresh-hint">
                        Codeforces 官方 API 限流严格，30
                        分钟内不建议重复刷新；全量更新会自动跳过近期已抓取的 CF 数据。
                      </div>
                      <div v-if="item.lastError && item.canViewError" class="sync-error">
                        {{ item.lastError }}
                      </div>
                    </div>
                    <div class="sync-side">
                      <div class="sync-counts">
                        <span
                          ><strong>{{ platformStats(item.platform).ac }}</strong> 题</span
                        >
                        <span>{{ platformStats(item.platform).submit }} 提交</span>
                        <span v-if="auditByPlatform(item.platform)"
                          >去重提交
                          {{ auditByPlatform(item.platform)?.distinctSubmitCount || 0 }}</span
                        >
                        <span v-if="auditByPlatform(item.platform)"
                          >AC 提交
                          {{ auditByPlatform(item.platform)?.acceptedSubmitCount || 0 }}</span
                        >
                        <span v-if="auditByPlatform(item.platform)">
                          本次 {{ auditByPlatform(item.platform)?.lastRawFetchedCount || 0 }} 原始 /
                          {{ auditByPlatform(item.platform)?.lastFetchedCount || 0 }} 有效
                        </span>
                        <span
                          v-if="
                            auditByPlatform(item.platform) &&
                            Number(auditByPlatform(item.platform)?.lastSkippedCount || 0) > 0
                          "
                        >
                        ...
                        <span
                          v-if="
                            auditByPlatform(item.platform) &&
                            Number(auditByPlatform(item.platform)?.filteredDuplicateCount || 0) > 0
                          "
                        >
                ...
                <template v-else-if="platformPeriodRows.length > 0">
                  <div
                    ...
                    v-for="item in platformPeriodRows"
                    :key="item.platform"
                  >
                    <div>
                      <div class="sync-platform">{{ platformLabel(item.platform) }}</div>
                    ...
                    <div class="sync-side">
                      <div class="sync-counts">
                        <span
                          ><strong>{{ item.ac.total }}</strong> 题</span
                        >
                ...
                <div
                  ...
                  v-if="
                    !loadingSyncStatus &&
                    syncStatusRows.length === 0 &&
                    platformPeriodRows.length === 0
                  "
                >
                  暂无 OJ 绑定，绑定后会显示数据同步状态。
                </div>
              </div>
            </div>
          </div>
          <div class="section achievement-section" style="position: relative">
            ...
            <div class="header">
              ...
              <div class="header-tabs achievement-header-actions">
                ...
                >
                <button
                  class="achievement-action-button achievement-view-all"
                  @click="showAchievementDrawer = true"
                >
                  查看全部 &gt;
                </button>
              </div>
            ...
            <div class="content">
              <div class="achievement-grid">
                <div
                  ...
                  v-for="badge in visibleAchievements"
                  :key="badge.key"
                  :class="[
                    { locked: !badge.unlocked, 'hidden-achievement': badge.hidden },
                    `tone-${badge.tone}`,
                  ]"
                >
                  ...
                  <div class="achievement-info">
                    ...
                    <div v-if="achievementGlobalRateLabel(badge)" class="achievement-global-rate">
                      {{ achievementGlobalRateLabel(badge) }}
                    </div>
              ...
              <div class="achievement-more" v-if="achievements.length > visibleAchievements.length">
                只展示精选成就，还有
                {{ achievements.length - visibleAchievements.length }} 个成就等你解锁。
              </div>
            </div>
          </div>
          <div class="section" style="position: relative">
          ...
          <div class="section" style="position: relative">
            ...
            <div class="header">
              ...
              <div class="header-tabs">
                <span class="tab" @click="router.push(`/allActivities?id=${user.userId}`)"
                  >查看所有动态</span
                >
              </div>
            </div>
            <div class="content">
              <div v-if="activities.length != 0" class="activities">
                <div
                  ...
                  :key="`${activity.link}-${activity.time}`"
                >
                  <div class="title">
          ...
          <div class="section" style="position: relative">
            ...
            <div class="header">
              ...
              <div class="header-tabs">
                <span class="tab" @click="router.push(`/contest?id=${user.userId}`)"
                  >查看所有比赛</span
                >
              </div>
            </div>
            <div class="content">
              <div class="contests">
                <div class="contest" v-for="contest in contests" :key="contest.id">
                  <div class="info">
          ...
          <div
            class="moblie-actions"
            v-if="currentUserId && (isSelfProfile || canShowProfileMessageAction)"
          >
            <template v-if="currentUserId === Number(user.userId)">
              <button class="btn def" @click="showUpdateConfirm">更新OJ数据</button>
              <button class="btn def" @click="router.push('/changeprofile')">编辑个人资料</button>
    ...
    <div
      v-if="showAchievementDrawer"
      class="achievement-drawer-mask"
      @click="showAchievementDrawer = false"
    >
      <aside class="achievement-drawer" @click.stop>
        <div class="achievement-drawer-header">
          ...
          <div class="achievement-drawer-count">
        ...
        <div class="achievement-drawer-list">
          ...
          <div class="achievement-empty" v-if="filteredAchievements.length === 0">
            暂无成就记录。
          </div>
        </div>
      </aside>
    ...
    <div v-if="detailModal.open" class="detail-modal-mask" @click="closePlatformDetail">
      <section class="detail-modal" ref="detailModalRef" @click.stop>
        ...
        <div class="detail-summary" v-if="platformDetail">
          <div>
            <strong>{{ platformDetail.summary.distinctAc }}</strong
            ><span>去重 AC</span>
          </div>
          <div>
            <strong>{{ platformDetail.summary.acceptedSubmits }}</strong
            ><span>AC 提交</span>
          </div>
          <div>
            <strong>{{ platformDetail.summary.distinctSubmitted }}</strong
            ><span>去重提交题</span>
          </div>
          <div>
            <strong>{{ platformDetail.summary.rawSubmits }}</strong
            ><span>原始提交</span>
          </div>
          <div>
            <strong>{{ platformDetail.summary.filteredDuplicate }}</strong
            ><span>去重过滤</span>
          </div>
          <div>
            <strong>{{ platformDetail.summary.filteredInvalid }}</strong
            ><span>异常记录</span>
          </div>
        </div>
        <div class="detail-tabs">
          <button
            class="achievement-action-button"
            :class="{ active: detailModal.mode === 'ac' }"
            @click="switchPlatformDetailMode('ac')"
          >
            AC 题列表
          </button>
          <button
            class="achievement-action-button"
            :class="{ active: detailModal.mode === 'submit' }"
            @click="switchPlatformDetailMode('submit')"
          >
            提交记录
          </button>
        </div>
        <div class="detail-table-wrap" ref="detailTableWrapRef">
          <table
            class="detail-table detail-table-problems"
            v-if="platformDetail && detailModal.mode === 'ac'"
          >
            <thead>
              <tr>
                <th>题目</th>
        ...
        <div class="detail-pagination" v-if="platformDetail">
          <button
            class="achievement-action-button"
            :disabled="detailModal.page <= 1"
            @click="changePlatformDetailPage(-1)"
          >
            上一页
          </button>
          <span
            >第 {{ detailModal.page }} 页 / 共
            {{ Math.max(1, Math.ceil(platformDetail.total / detailModal.pageSize)) }} 页</span
          >
          <button
            class="achievement-action-button"
            :disabled="detailModal.page >= Math.ceil(platformDetail.total / detailModal.pageSize)"
            @click="changePlatformDetailPage(1)"
          >
            下一页
          </button>
...
import {
  buildTrainingPortrait,
  buildTrainingStatuses,
  type TrainingPortrait,
  type TrainingStatusBadge,
...
const updateConfirmMessage = computed(() => {
  const hasCodeforces = Boolean(
    user.value.links.CodeForces ||
    user.value.spiders?.some((item) => item.platform === "CodeForces"),
  )
  if (!hasCodeforces) {
...
const isSelfProfile = computed(
  () => currentUserId.value !== 0 && currentUserId.value === Number(user.value.userId),
)
...
const teamOwner = computed(() =>
  teamInfo.value.members.find((member) => Number(member.userId) === Number(teamInfo.value.ownerId)),
)
const syncStatusRows = computed(() => {
  return [...syncStatuses.value].sort(
    (a, b) => platformOrderIndex(a.platform) - platformOrderIndex(b.platform),
  )
})
...
const permanentAchievementStorageKey = computed(
  () => `wust-achievements:${Number(user.value.userId || 0)}`,
)
...
const passwordChangeStorageKey = computed(
  () => `wust-password-change-count:${Number(user.value.userId || 0)}`,
)
...
const rawAchievements = computed<AchievementBadge[]>(() => {
  return buildAchievementBadges(
    profilePeriodData.value,
...
const achievements = computed<AchievementBadge[]>(() => {
  ...
  return rawAchievements.value.map((item) =>
    permanentSet.has(item.key) ? { ...item, unlocked: true, progress: 100 } : item,
  )
})
...
const saveAchievementSnapshot = () => {
  ...
  if (
    snapshotSavePaused.value ||
    ["queued", "running"].includes(activeSpiderJob.value?.status || "")
  )
    return
...
const filteredAchievements = computed(() => {
  ...
  if (achievementFilter.value === "progress")
    return achievements.value.filter((item) => !item.unlocked && !item.hidden)
  if (achievementFilter.value === "hidden") return achievements.value.filter((item) => item.hidden)
  return [...achievements.value].sort(
    (a, b) => Number(b.unlocked) - Number(a.unlocked) || Number(a.hidden) - Number(b.hidden),
  )
})
...
const loadAchievementGlobalRates = async () => {
  ...
  try {
    ...
    if (
      cached?.generatedAt &&
      Date.now() - Number(cached.generatedAt) < cacheTtl &&
      cached?.rates
    ) {
      achievementGlobalRates.value = cached.rates
      achievementNightAcPercentile.value = Number(
        cached.nightPercentiles?.[Number(user.value.userId)] || 0,
      )
  ...
  try {
    ...
    achievementNightAcPercentile.value = Number(
      response.data.nightPercentiles?.[snapshotUserId] || 0,
    )
...
const getTeamInfo = async () => {
  ...
  const memberIds = teamResponse.data.users
    .map((member) => Number(member.userId))
    .filter((id) => Number.isFinite(id) && id > 0)
  const teamStatsResponse =
    memberIds.length > 0 ? await API.core.statistic.teamPeriod(memberIds) : null
  const statsByUserId = new Map(
    (teamStatsResponse?.success ? teamStatsResponse.data.members : []).map((item) => [
      Number(item.userId),
      item,
    ]),
  )
  ...
  const sortedMembers = members.sort(
    (a, b) => b.acTotal - a.acTotal || b.submitTotal - a.submitTotal || a.userId - b.userId,
  )
  ...
  teamInfo.value = {
    ...
    acTotal: teamTotalStats
      ? toSafeNumber(teamTotalStats.ac?.total)
      : sortedMembers.reduce((sum, item) => sum + item.acTotal, 0),
    submitTotal: teamTotalStats
      ? toSafeNumber(teamTotalStats.submit?.total)
      : sortedMembers.reduce((sum, item) => sum + item.submitTotal, 0),
...
const sendInvite = async (inviteeId: number) => {
  ...
  if (response.success) {
    inviteCandidates.value = inviteCandidates.value.filter(
      (item) => Number(item.userId) !== targetUserId,
    )
  }
}
...
const transferOwner = async (member: TeamMember) => {
  ...
  const confirmed = window.confirm(
    `确定要把团队「${teamInfo.value.name}」的队长转移给 ${displayName} 吗？转移后你将不再拥有团队管理权限。`,
  )
  if (!confirmed) {
    return
  }
...
const leaveTeam = async () => {
  ...
  const confirmed = window.confirm(
    `确定要退出团队「${teamInfo.value.name}」吗？退出后需要重新接受邀请才能加入团队。`,
  )
  if (!confirmed) {
    return
  }
...
const disbandTeam = async () => {
  ...
  const confirmed = window.confirm(
    `确定要解散团队「${teamInfo.value.name}」吗？所有成员都会变为无团队，待处理邀请会失效。`,
  )
  if (!confirmed) {
    return
  }
...
const isRefreshingPlatform = (platform: string) => {
  if (!activeSpiderJob.value || !["queued", "running"].includes(activeSpiderJob.value.status))
    return false
  return (
    !activeSpiderJob.value.currentPlatform || activeSpiderJob.value.currentPlatform === platform
  )
}
...
const platformRefreshCooldownMs = (platform: string) => {
  return isCodeforcesPlatform(platform)
    ? codeforcesRefreshCooldownMs
    : defaultPlatformRefreshCooldownMs
}
...
.achievement-card.locked {
  ...
  background-color: color-mix(
    in srgb,
    var(--section-background-color) 88%,
      ...
      > .left {
        ...
        > .background {
          ...
          background:
            radial-gradient(
              circle at 30% 20%,
              color-mix(in srgb, var(--neon-cyan) 18%, transparent),
              transparent 34%
            ),
