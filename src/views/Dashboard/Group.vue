    ...
    <div class="section">
      <div class="header">
        ...
        <div class="header-tabs">
          <span class="tab" @click="showAddGroup = !showAddGroup">{{
            showAddGroup ? "关闭" : "创建组"
          }}</span>
      ...
      <div class="content">
        ...
        <table style="z-index: 1">
          ...
          <tr v-for="(item, index) in groupData?.list" :key="item.id">
            <td>{{ item.id }}</td>
        ...
        <div class="pageNavigation" v-if="groupData">
          <div class="group">
            ...
            <div class="pageButtons">
              <button
                v-for="value in groupPages"
                :key="value"
                :class="value === groupData.currentPage ? 'active' : ''"
                @click="value === groupData.currentPage ? null : getGroupData(value)"
              >
                {{ value }}
              </button>
            </div>
    ...
    <transition name="fade">
      <div class="section" v-show="usersData.id != -1">
        <div class="header">
          ...
          <div class="header-tabs">
            <span class="tab" @click="closeGroupUsers">退出组管理</span>
            <span class="tab" @click="showAddUser ? cancelAddUser() : (showAddUser = true)">{{
              showAddUser ? "取消添加成员" : "添加成员"
            }}</span>
        ...
        <div class="content">
          ...
          >
            ...
            <div v-if="searchUserData.length > 0">
              <table>
                ...
                <tr v-for="item in searchUserData" :key="item.userId">
                  ...
                  <td>
                    <div class="actions">
                      <button
                        ...
                        @click="move(item.userId, usersData.id)"
                        v-if="
                          usersData.users.findIndex((user) => user.userId === item.userId) === -1
                        "
                      >
                        添加该用户
                      </button>
          ...
          <table style="margin-bottom: 10px" v-if="usersData.users.length > 0">
            ...
            <tr v-for="item in usersData?.users" :key="item.userId">
              <td>{{ item.userId }}</td>
              <td>{{ item.name }}</td>
...
const openGroupUsers = (groupId: number) => {
  getUsersData(Number(groupId))
  cancelAddUser()
}

const closeGroupUsers = () => {
  usersData.value.id = -1
  cancelAddUser()
}
