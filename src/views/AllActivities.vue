    ...
    <div class="section">
      <div class="header">
        ...
        <div class="header-tabs">
          <span class="tab" @click="router.push(`/allActivities`)" v-if="userId != -1"
            >查看大家的动态</span
          >
        ...
        <div v-else-if="activities.length != 0" class="activities">
          <div class="activity" v-for="(activity, index) in activities" :key="index">
            <div class="title">
              ...
              <span>
                <router-link :to="`/profile?id=${activity.userId}`" class="name-link">{{
                  activity.name
                }}</router-link>
                {{ activity.title }}
              </span>
...
const getNewSubmit = async (currentCursor: number) => {
  ...
  if (response.success && response.data.data.length > 0) {
    ...
    if (response.data.data.length > 0) {
      // 更新游标为最后一条数据的时间戳
      const lastItem: CoreSubmitLogGetByIdData | undefined =
