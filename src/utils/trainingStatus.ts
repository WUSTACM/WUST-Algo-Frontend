...
const topPlatforms = (logs: CoreSubmitLogGetByIdData[]): string[] => {
  ...
  return PLATFORM_ORDER.filter((platform) => (counts.get(platform) || 0) > 0).slice(0, 2)
}
...
export const buildTrainingStatuses = ({
  ...
  if (totalSubmit === 0 || (inactiveDays !== null && inactiveDays >= 10)) {
    pushUnique(badges, {
      label: "疑似掉队",
      description:
        inactiveDays === null ? "暂无提交记录" : `已约 ${Math.floor(inactiveDays)} 天没有提交`,
      tone: "warning",
    })
  }
  ...
  if (
    !badges.some((item) => item.label === "疑似掉队") &&
    thisWeekSubmit >= 8 &&
    thisMonthAc >= 10
  ) {
    pushUnique(badges, {
