...
export default class Analyse {
  static period = (data: CoreStatisticPeriodData) => {
      ...
      if (weekTrend > 10) {
        analyse.push(
          Random.choice([
            ...
            weekTrendRate > 0
              ? `🔥 本周 AC 数增长 ${weekTrendRate.toFixed(0)}%，飞速进步！`
              : `🔥 本周 AC 数增长 ${weekTrend} 题，太棒了！`,
          ]),
        )
      } else if (weekTrend > 5) {
        analyse.push(
      ...
      if (monthTrend > 30) {
        analyse.push(
          Random.choice([
            ...
            monthTrendRate > 0
              ? `🔥🔥 本月 AC 数增长 ${monthTrendRate.toFixed(0)}%，突飞猛进！`
              : `🔥🔥 本月 AC 数增长 ${monthTrend} 题，太强了！`,
          ]),
        )
    ...
    // } else {
    analyse.push(
      "🔗 推荐：<a href='https://www.nowcoder.com/problem/tracker#/daily'>每日一题保持手感</a>",
    )
    // }
